...
my $rh_siteMaxStranded = $rh_bigtables->{max_stranded_posRatio}; # major allele 
my $rh_siteSecStranded = $rh_bigtables->{second_stranded_posRatio}; # minor allele (ranked 2nd)
...
for (my $i = 0; $i < @samples; $i ++){
	...
	foreach my $p (@positions){
		...
		($rh_siteMinorDep->{$samples[$i]}->{$p} ne 'na' && $rh_siteMinorDep->{$samples[$i]}->{$p} >= $THRES_minorAlleDepCut) ) {
			...
			}  else { #if ($rh_siteMinorFreq->{$samples[$i]}->{$p} < $THRES_iSNVCut) {
				my @patt = split //, $rh_bigtables->{ref_max_sec}->{$samples[$i]}->{$p};
				# ref:max-sec, patt[0]==ref, patt[2]==max
				if ($patt[0] ne $patt[2] &&  
...
#			} else { # for example, ref=G, max=A:90(20/70), sec=G:3(2/1)
			# **********************************************************
			# 之前的代码没有包括所有的情况，在上述判断条件下，会有都不
			# 满足的位点被漏掉。整个流程需要进行重构
			# **********************************************************
			}
		}
